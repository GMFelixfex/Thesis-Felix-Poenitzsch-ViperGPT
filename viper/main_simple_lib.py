...
def get_code(query, code_gen_ai='codex'):
    if config.codex.model == 'codellama':
        model_name_codex = 'codellama' 
    else:
        model_name_codex =   'codex'
    code = forward(code_gen_ai, prompt=query, input_type="image")
