    ...
    def get_qa(self, prompts, prompt_base: str = None) -> list[str]:
        ...
        if self.n_votes > 1:
            ...
            for i in range(len(response.choices)):
                responselist.append(response.choices[i].message.content)
            #if self.model == 'chatgpt':
...
def codex_helper(extended_prompt):
    ...
    if config.codex.model in ("gpt-4", "gpt-3.5-turbo"):
        ...
        responses = [client2.chat.completions.create(
        ...
        resp = []
        for r in responses:
            message_content = r.choices[0].message.content
            new_message_content = message_content.replace("execute_command(image)", 
                                                        "execute_command(image, my_fig, time_wait_between_lines, syntax)")
            resp.append(new_message_content)
...
class CodexModel(BaseModel):
    ...
    def forward(self, prompt, input_type='image', prompt_file=None, base_prompt=None, extra_context=None):
        print("Codex forward")
        print(prompt , input_type, prompt_file, base_prompt, extra_context)
        ...
        if isinstance(prompt, list):
            ...
            testprompt = base_prompt.replace("INSERT_QUERY_HERE", prompt)

            if input_type is not None:
                testprompt = testprompt.replace('INSERT_TYPE_HERE', input_type)
            
            if extra_context is not None:
                testprompt = testprompt.replace('EXTRA_CONTEXT_HERE', str(extra_context))
            ...
            extended_prompt = [testprompt]

        else:
            raise TypeError("prompt must be a string or a list of strings")
        
        result = self.forward_(extended_prompt)
        print("Codex forward result")
...
# @cache.cache
@backoff.on_exception(backoff.expo, Exception, max_tries=2)
def lm_studio_helper(extended_prompt):
    #assert 0 <= config.lm_studio.temperature <= 1
    #assert 1 <= config.lm_studio.best_of <= 20
    
    #print("lm studio helper prompt")

    
    prompt = extended_prompt.replace("\n", " ")
    response = client.chat.completions.create(
    ...
    #print("lm studio helper prompt end")
    #print(response.choices[0].message.content)
    
    

    #print("lm studio helper loop")
    message_content:str = response.choices[0].message.content

    if "execute_command(image)" in message_content:
        message_content = "def execute_command(image)" + message_content.split("execute_command(image)")[1]
        message_content = message_content.replace("execute_command(image)", 
                                                "execute_command(image, my_fig, time_wait_between_lines, syntax)")
    else:
        return message_content
    if r"\end{code}" in message_content:
        message_content = message_content.split(r"\end{code}")[0]


    #print("lm studio helper message_content")
    #new_message_content = message_content.split(r"\begin{code}")[1]
    #print("lm studio helper message_content split1")
    
    #new_message_content1 = new_message_content.split(r"\end{code}")[0]
    #print("lm studio helper message_content split2")
    #new_message_content2 = new_message_content1.replace("execute_command(image)", 
    #                                            "execute_command(image, my_fig, time_wait_between_lines, syntax)")
    #print("lm studio helper message_content replace")
...
'''
    #resp = []
    #print("lm studio helper loop")
    #message_content:str = testprompt

    #new_message_content1 = message_content.replace("execute_command(image)", 
    #                                            "execute_command(image, my_fig, time_wait_between_lines, syntax)")
    ##print("lm studio helper message_content replace")
    #resp.append(message_content2)
...
class LmStudioModel(BaseModel):
    ...
    def __init__(self, gpu_number=0):
        super().__init__(gpu_number=gpu_number)
        with open(config.lm_studio.prompt) as f:
            self.base_prompt = f.read().strip()
        self.fixed_code = None
        if config.use_fixed_code:
            with open(config.fixed_code_file) as f:
    ...
    def forward(self, prompt, input_type='image', prompt_file=None, base_prompt:str=None, extra_context=None):
        #print("lm_studio forward")
        #print(prompt , input_type, prompt_file, base_prompt, extra_context)
...
#                base_prompt = f.read().strip()
        ...
        #print("lm_studio forward prompt")
        ...
        #result = self.forward_(extended_prompt)
        #print("lm_studio forward preresult")
        result = lm_studio_helper(extended_prompt)
        #print("lm_studio forward result")


        return result

    def forward_(self, extended_prompt):
        #print("lm_studio forward_")
        if len(extended_prompt) > self.max_batch_size:
           # print("len(extended_prompt) > self.max_batch_size")
            response = []
            for i in range(0, len(extended_prompt), self.max_batch_size):
                response += self.forward_(extended_prompt[i:i + self.max_batch_size])
            return response
        try:
            #print("len(extended_prompt) ", len(extended_prompt))
            response = lm_studio_helper(extended_prompt)
        except openai.RateLimitError as e:
            print("RateLimitError")
            #print(len(extended_prompt))
            if len(extended_prompt) == 1:
                warnings.warn("This is taking too long, maybe server is offline?")
            # Will only be here after the number of retries in the backoff decorator.
            # It probably means a single batch takes up the entire rate limit.
            sub_batch_1 = extended_prompt[:len(extended_prompt) // 2]
            sub_batch_2 = extended_prompt[len(extended_prompt) // 2:]
